...
def test_botoes(driver):
    ...
    xpath_secondary = (
        "//*[@id='secondary-btn' and contains(text(), 'Botão Secundário')]"
    )
    WebDriverWait(driver, 10).until(
...
def test_formularios(driver):
    ...
    try:
        ...
        driver.find_element(By.ID, "number-input").send_keys(
        ...
        driver.execute_script(
            "arguments[0].value = arguments[1];", color_input, new_color
        )
        ...
        driver.find_element(
            By.XPATH, "//*[@type='checkbox' and @value='opcao2']"
        ).click()
        driver.find_element(
            By.XPATH, "//*[@type='checkbox' and @value='opcao1']"
        ).click()
        ...
        assert sorted([opt.text for opt in multi_select.all_selected_options]) == [
            "Opção 1",
            "Opção 3",
        ]
        ...
        form_data = (
            ...
            .until(EC.visibility_of_element_located((By.ID, "form-data")))
            .text
        )
    ...
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//*[contains(text(), 'Próximo')]"))
    ).click()
...
def test_alertas(driver):
    ...
    alert_warning = WebDriverWait(driver, 10).until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, ".alert.alert-warning"))
    )
...
def full_execution():
    ...
    This replaces the `@browser_session` decorator from pyminima.
    """
